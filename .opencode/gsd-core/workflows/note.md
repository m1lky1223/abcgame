...
- **Global scope**: `/Users/rony/dev/abcgame/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/rony/dev/abcgame/.opencode/notes/`)
...
2. Glob `/Users/rony/dev/abcgame/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/Users/rony/dev/abcgame/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/rony/dev/abcgame/.opencode/notes/` — works in any directory
